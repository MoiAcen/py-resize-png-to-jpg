...
# 「無標籤檔案快速分類」一次列出幾組。切出來的組別動輒上百，全印出來反而看不到重點，
# 所以只列體積最大的前幾組；沒列到的下次再進來就會浮上來。
RULE_TAG_SHOW_N          = 30
EXTRA_ANALYZE_DEFAULT    = 500    # 選單「追加解析」預設要補幾筆（直接 Enter 用這個值）
MAX_TARGET_FILES         = 5000   # 分析目標上限包數
# 每輪最多「新解析」幾個包。掃到這個數量就停止開新檔案、直接出排行榜，
# 下一輪再從沒掃過的繼續往下——這樣收藏庫大的時候不會一次跑到天荒地老，
# 快取也會一輪一輪把它吃完。想一次多做一點就調大，或用選單的「追加解析」指定筆數。
# 設 0 表示不限制（掃到 MAX_TARGET_FILES 或整個掃完為止）。
